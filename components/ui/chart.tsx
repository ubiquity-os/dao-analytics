...
>(
  (
    ...
    const tooltipLabel = React.useMemo(() => {
      ...
      const analytics = item.payload as IssueAnalytics;
      ...
      return (
        <div className="grid gap-1.5  rounded-lg p-1.5 text-xxs">
          {timeFromOpenToClose && timeFromPrOpenToIssueClose && user && (
            <div>
              {user.login} authored the PR
              <br />
              {ms(timeFromOpenToClose, { long: true })} days to close the task
              <br />
              {ms(timeFromPrOpenToIssueClose, { long: true })} days to merge the PR
            </div>
          )}
        </div>
      );
    ...
    return (
      <div
        ...
        onClick={() => window.open(payload[0].payload.url, "_blank")}
      >
